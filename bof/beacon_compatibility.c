...
// must be here not in .h file
// https://pkg.go.dev/cmd/cgo#hdr-C_references_to_Go
extern void GoBeaconOutput(void* data);

void RunBof(void* Function, void* Buffer, u_int64 Length) {
    ((void(*)())Function)(Buffer, Length);
}
...
void BeaconPrintf(int type, char* fmt, ...) {
    ...
    va_start(args, fmt);
    (void)vsnprintf(buf, length, fmt, args);
    va_end(args);
    GoBeaconOutput(buf);
    return;
}

void* GetBeaconOutput(){
    return BeaconOutput;
}
